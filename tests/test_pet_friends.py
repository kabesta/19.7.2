...
def test_add_new_pet_with_valid_data(name='Кошечка', animal_type='кошка',
                                     age='2', pet_photo='images/kot1.jpg'):
...
def test_add_new_create_pet_simple_with_valid_data(name='Барсик', animal_type='дворянин', age='3'):
...
def test_add_new_pets_set_photo_pet_id_with_valid_data(pet_photo='images/kot2.jpg'):
    """Проверяем что можно добавить фото питомца """

    # Получаем полный путь изображения питомца и сохраняем в переменную pet_photo
    pet_photo = os.path.join(os.path.dirname(__file__), pet_photo)

    # Запрашиваем ключ api и сохраняем в переменную auth_key и список моих питомцев
    _, auth_key = pf.get_api_key(valid_email, valid_password)
...
def test_add_new_pet(name='//////', animal_type='******', age='Four', pet_photo='images/kot3.jpg'):
...
def test_successful_delete_self_pet():
    ...
    if len(my_pets['pets']) == 0:
        pf.add_new_pet(auth_key, "Суперкот", "кот", "3", "images/kot1.jpg")
        _, my_pets = pf.get_list_of_pets(auth_key, "my_pets")
